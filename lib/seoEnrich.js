...
export function stripArticleLinks(html) {
  ...
  // Remove data-injected paragraphs (Related:, mid-content inserts)
  ...
  // Remove data-injected h3 sections + the ul that follows
  // Handles: Related Articles, Explore More
  result = result.replace(
    /<h3\s[^>]*data-injected="true"[^>]*>[\s\S]*?<\/h3>\s*(<ul\s[^>]*data-injected="true"[^>]*>[\s\S]*?<\/ul>)?/gi,
    ''
  );

  // Remove legacy (non-attributed) Related Articles / Read More / Explore More h3+ul
  result = result.replace(
    /<h3>(Related Articles|Read More|Explore More)<\/h3>\s*<ul>[\s\S]*?<\/ul>/gi,
    ''
  );

  // Unwrap all article-link anchors → plain text (any category path)
  result = result.replace(/<a\s[^>]*class="article-link"[^>]*>([\s\S]*?)<\/a>/gi, '$1');
  result = result.replace(/<a\s[^>]*href="\/[^"]*"[^>]*class="article-link"[^>]*>([\s\S]*?)<\/a>/gi, '$1');
