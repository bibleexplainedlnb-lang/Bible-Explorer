...
// Fallback topic pages — used when no matching article exists for a keyword group
const TOPIC_FALLBACK_URLS = {
  faith:       '/topics/faith/',
  fear:        '/topics/fear/',
...
// ─── Step 2: Inject inline article/topic links (max 5) ────────────────────
// Prefers linking to a matching article; falls back to the topic page.
export function injectInlineLinks(html, publishedArticles) {
  ...
  // Build best URL per keyword group: article first, topic page as fallback
  const groupToUrl = {};
  for (const groupName of Object.keys(KEYWORD_GROUPS)) {
    const article = findArticleForGroup(groupName, publishedArticles || []);
    if (article) {
      groupToUrl[groupName] = `/bible-verses/${article.slug}/`;
    } else if (TOPIC_FALLBACK_URLS[groupName]) {
      groupToUrl[groupName] = TOPIC_FALLBACK_URLS[groupName];
    }
  }
  ...
  const resultHtml = tokens.map(token => {
    ...
    for (const { groupName, variation } of VARIATION_LIST) {
      ...
      const { text: newText, replaced } = replaceFirstVariation(text, variation, url);
      if (replaced) {
        text = newText;
        usedGroups.add(groupName);
        if (url.startsWith('/bible-verses/')) usedSlugs.add(url.replace('/bible-verses/', '').replace(/\/$/, ''));
        linksAdded++;
      }
    }
    return text;
...
// Removes article-to-article links, Know More blocks, Related/Read More sections
// so enrichContent() can be re-run cleanly on existing content.
export function stripArticleLinks(html) {
  if (!html) return html;

  let result = html;

  // Remove Know More blocks: <p class="know-more-block">...</p>
  ...
  // Unwrap article-link anchors: <a href="/bible-verses/..." class="article-link">text</a> → text
  ...
  // Unwrap topic-fallback article-links (href="/topics/...")
  ...
  // Unwrap Bible verse links: <a href="/bible/..." class="verse-ref">Ref</a> → Ref
  // But keep the surrounding parentheses pattern intact
