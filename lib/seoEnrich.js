...
function buildGroupUrls(publishedArticles) {
  ...
  for (const groupName of Object.keys(KEYWORD_GROUPS)) {
    ...
    // 2. Topic fallback page
    if (TOPIC_FALLBACK_URLS[groupName]) { groupToUrl[groupName] = TOPIC_FALLBACK_URLS[groupName]; continue; }
    ...
    // 4. Any matching article as last resort
...
async function injectInlineLinksAI(html, publishedArticles) {
  ...
  for (const { groupName, variation } of VARIATION_LIST) {
    if (keywordCount + contextualCount >= MAX_TOTAL) break;
    ...
    if (foundInContent) {
      if (isKeywordSlot) {
        ...
        if (replaced) { result = newHtml; linked = true; contextualCount++; }
          ...
          // Contextual expansion failed — fall back to simple link
      ...
      // Keyword absent — AI-generate and inject a sentence
...
export async function enrichContent(html, publishedArticles, category, currentSlug) {
  ...
  // 2. Hybrid AI inline links
  const { html: linkedHtml, usedSlugs: inlineSlugs } = await injectInlineLinksAI(result, publishedArticles || []);
  result = linkedHtml;
