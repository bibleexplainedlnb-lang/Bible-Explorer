...
// ─── URL builder — ONLY links to verified published articles ───────────────
// Returns null for a group when no published article exists (never falls back
// to a topic page that may not exist in the DB).
function buildGroupUrls(publishedArticles) {
  ...
  for (const groupName of Object.keys(KEYWORD_GROUPS)) {
    ...
    // 4. Any matching published article as last resort
