...
/**
 * Classify a topic name as 'question' | 'person' | 'concept'.
 *
 * - question : starts with a question/auxiliary word (how, why, what, …)
 * - person   : 1–3 proper-noun words with no stop-words (e.g. "King David", "Paul")
 * - concept  : everything else (faith, prayer, anxiety, love, …)
 */
export function classifyTopic(name = '') {
  ...
  if (
    ...
    !words.some(w => PERSON_STOP_WORDS.has(w.toLowerCase()))
  ) {
    return 'person';
  }
 ...
 * Uses the DB category as primary signal, but falls back to text-based
 * classification to catch mislabelled topics.
 */
export function buildTitleHint(category, topicName) {
  const name = topicName.trim();
  const textType = classifyTopic(name);

  // DB category takes priority for these two explicit categories
  if (category === 'bible-characters' || textType === 'person') {
  ...
  if (textType === 'question') {
    ...
    return `\nIMPORTANT: title must be "${capitalized}" or "${capitalized} According to the Bible" — choose whichever reads better; slug must reflect the chosen title`;
  }

  // concept — pick template based on category
  if (category === 'topics') {
  ...
  // guides, teachings, questions (non-question text), fallback — let AI decide
  return '';
}
