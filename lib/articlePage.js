...
async function fetchRelated(category, currentSlug) {
  ...
  const { data: topicRows } = await supabase.from('topics').select('id').eq('category', category);
...
export function makeArticlePage(category) {
  return async function ArticlePage({ params }) {
    ...
    const related  = await fetchRelated(category, params.slug);
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {related.map(r => (
                  <li key={r.slug} style={{ padding: '0.35rem 0', borderBottom: '1px solid #ede8dc' }}>
                    <Link
                      href={articleUrl(r.slug, r.topics?.category)}
