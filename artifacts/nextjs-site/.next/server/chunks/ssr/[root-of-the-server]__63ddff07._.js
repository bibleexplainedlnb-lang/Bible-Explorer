...
function getTopicQuestions(topic) {
    if (topic.questions && topic.questions.length > 0) {
        return topic.questions.map((s)=>questions.find((q)=>q.slug === s)).filter((q)=>q !== undefined);
    }
    return questions.filter((q)=>q.topic.toLowerCase() === topic.name.toLowerCase());
}
function getGuideForTopic(topicSlug) {
